...
export function AddContactPersonModal({ isOpen, onClose, onAdd }: AddContactPersonModalProps) {
  ...
  const [selectedExisting, setSelectedExisting] = React.useState<LinkedContact | null>(null);
  ...
  const isValid = mode === "existing" ? !!selectedExisting : form.firstName.trim().length > 0 && form.lastName.trim().length > 0;

  const handleAdd = () => {
    if (!isValid) return;
    onAdd(mode === "existing" ? selectedExisting! : createContactFromForm(form));
    resetAndClose();
  };
    ...
    <Dialog.Root open={isOpen} onOpenChange={resetAndClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white border border-[#ccc] shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)] w-[480px] max-h-[85vh] flex flex-col z-[10000] outline-none"
        >
          <div className="pt-[24px] px-[24px] pb-px shrink-0">
          ...
          <div className="p-[24px] overflow-y-auto">
            <label className="flex items-center gap-[10px] cursor-pointer mb-[12px]">
              <input
                type="radio"
                name="add-contact-mode"
                checked={mode === "existing"}
                onChange={() => setMode("existing")}
                className="w-[16px] h-[16px] accent-[#1a1a1a]"
              />
              <span className="font-['Roboto:Regular',sans-serif] text-[14px] text-[#1a1a1a]">Existing contact</span>
            </label>
            {mode === "existing" && (
              <div className="pl-[26px] mb-[12px]">
                <label className="block font-['Roboto:Regular',sans-serif] text-[13px] text-[#666666] mb-[4px]">
                  Search existing contact
                </label>
                <ContactSearchDropdown onSelect={setSelectedExisting} />
              </div>
            )}
            ...
            <label className="flex items-center gap-[10px] cursor-pointer">
              <input
                ...
                className="w-[16px] h-[16px] accent-[#1a1a1a]"
              />
              <span className="font-['Roboto:Regular',sans-serif] text-[14px] text-[#1a1a1a]">New contact</span>
            </label>
            {mode === "new" && (
              <div className="pl-[26px] pt-[12px]">
          ...
          <div className="flex items-center justify-end gap-[8px] px-[24px] py-[16px] border-t border-[#e5e5e5] shrink-0">
            <button
            ...
            <button
              onClick={handleAdd}
              disabled={!isValid}
              className="bg-[#1c7862] h-[30px] px-[16px] rounded-[33554400px] border border-[#1c7862] hover:bg-[#248E73] hover:border-[#248E73] disabled:bg-[#ccc] disabled:border-[#ccc] disabled:cursor-not-allowed transition-colors cursor-pointer"
            >
