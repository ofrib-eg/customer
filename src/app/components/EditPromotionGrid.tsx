...
const InputField: React.FC<{ label: string; value: string; onChange: (val: string) => void; type: 'date' | 'time' }> = ({ label, value, onChange, type }) => {
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'a') {
      e.currentTarget.select();
    }
...
interface EditPromotionGridProps {
...
export function EditPromotionGrid({ showReference, onShowReferenceChange, onModified, saveTrigger, onSave, onCancel }: EditPromotionGridProps) {
  ...
  const [dateFrom, setDateFrom] = useState("15.02.2026");
  const [timeFrom, setTimeFrom] = useState("00:00");
  const [dateTo, setDateTo] = useState("22.02.2026");
  ...
  useEffect(() => {
    onModified?.(modifiedCount);
  }, [modifiedCount, onModified]);
    ...
    <div className={`flex flex-col min-w-0 ${isStatic ? "pointer-events-none" : ""}`}>
          ...
          <div className="flex items-center w-full h-[42px] border-b border-[#CCCCCC] bg-white">
            ...
            <div className={`flex-1 h-full flex items-center ${isStatic ? "pl-2 pr-[5px]" : "px-[5px]"}`}>
              {isStatic ? (
                ...
                <InputField label="Date" value={dateFrom} onChange={(v) => { setDateFrom(v); trackChange(); }} type="date" />
              )}
            </div>
            <div className={`flex-1 h-full flex items-center ${isStatic ? "pl-2 pr-[5px]" : "px-[5px]"}`}>
              {isStatic ? (
                ...
                <InputField label="Time" value={timeFrom} onChange={(v) => { setTimeFrom(v); trackChange(); }} type="time" />
              )}
            </div>
          </div>
          ...
          <div className="flex items-center w-full h-[42px] border-b border-[#CCCCCC] bg-white">
            ...
            <div className={`flex-1 h-full flex items-center ${isStatic ? "pl-2 pr-[5px]" : "px-[5px]"}`}>
              {isStatic ? (
                ...
                <InputField label="Date" value={dateTo} onChange={(v) => { setDateTo(v); trackChange(); }} type="date" />
              )}
            </div>
            <div className={`flex-1 h-full flex items-center ${isStatic ? "pl-2 pr-[5px]" : "px-[5px]"}`}>
              {isStatic ? (
                ...
                <InputField label="Time" value={timeTo} onChange={(v) => { setTimeTo(v); trackChange(); }} type="time" />
              )}
            </div>
          </div>
              ...
              {isStatic ? (
                ...
                <div className="relative w-full h-[30px] flex items-center">
                  <select 
                    ...
                    value={reasonCode}
                    onChange={(e) => { setReasonCode(e.target.value); trackChange(); }}
                  >
                ...
                <div className="flex-[2] min-w-0 h-full flex items-center px-[5px]">
                  <div className="flex items-center">
                    <div 
                      ...
                      onClick={() => { setIsMemberOffer(!isMemberOffer); trackChange(); }}
                    >
                      {isMemberOffer && (
      ...
      <div className="flex flex-col">
        ...
        <PriceRow 
          ...
          onChange={(val) => { setOrdinaryNetPrice(val); trackChange(); }}
          isInput={!isStatic}
          hideLabel={isStatic} 
        />
        <PriceRow 
        ...
        <PriceRow 
          ...
          onChange={(val) => { setPromotionNetPrice(val); trackChange(); }}
          isInput={!isStatic}
          noBorder 
          hideLabel={isStatic} 
        />
      ...
      <div className="flex flex-col">
        ...
        <PriceRow 
          ...
          hasPercentInput={!isStatic} 
          isInput={!isStatic} 
          onChange={(val) => { setGrossProfitAmount(val); trackChange(); }}
          onPercentChange={(val) => { setGrossProfitPercent(val); trackChange(); }}
          hideLabel={isStatic} 
        />
        <PriceRow 
          ...
          isInput={!isStatic} 
          onChange={(val) => { setRetailPriceExclVat(val); trackChange(); }}
          hideLabel={isStatic} 
        />
        <PriceRow 
          label="VAT (1)" 
          ...
          isInput={!isStatic}
          onChange={(val) => { setVatAmount(val); trackChange(); }}
          hideLabel={isStatic} 
        />
        <PriceRow 
          label="Retail price" 
          ...
          isInput={!isStatic} 
          onChange={(val) => { setRetailPrice(val); trackChange(); }}
          hideLabel={isStatic} 
        />
        <PriceRow 
          ...
          renderMiddleContent={!isStatic ? (
            <div className="flex items-center h-full">
              <div 
                ...
                onClick={() => { setIsRounded(!isRounded); trackChange(); }}
              >
                {isRounded && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <svg className="size-[12px]" viewBox="0 0 12 12" fill="none">
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="flex-1 overflow-y-auto selection:bg-[#373737] selection:text-white">
        <div className="flex h-full min-w-0">
          <div className="flex-1 min-w-0 flex flex-col">
            {renderContent(false)}
          </div>
          {showReference && (
            <div className="w-[300px] border-l-2 border-[#FFFFFF] flex flex-col shrink-0 bg-[#F4F6F7]">
              {renderContent(true)}
            </div>
          )}
        </div>
